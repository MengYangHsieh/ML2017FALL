...
def main():
    ...
    # training data
    train_pixels, train_labels = extract_training_data(args.train)
    ...
    train_pixels = np.array(
        train_pixels, dtype=np.float64).reshape((len(train_pixels), 48, 48, 1))
    train_labels = np.array(train_labels, dtype=np.float64)

    train_pixels /= 255

    shuffle_index = np.arange(len(train_pixels))
    np.random.seed(20171111)
    np.random.shuffle(shuffle_index)
    train_pixels = train_pixels[shuffle_index]
    train_labels = train_labels[shuffle_index]

    # validation data
    num_validating_pixels = train_pixels.shape[0] // 10
    valid_pixels = train_pixels[:num_validating_pixels]
    train_pixels = train_pixels[num_validating_pixels:]
    valid_labels = train_labels[:num_validating_pixels]
    train_labels = train_labels[num_validating_pixels:]
